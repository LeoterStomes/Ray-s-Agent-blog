...
TOOLS_SCHEMA = [
    {"type":"function","function":{"name":"search_articles","description":"Search blog articles by keyword","parameters":{"type":"object","properties":{"query":{"type":"string","description":"Search keyword"},"limit":{"type":"integer","description":"Max results, default 3","default":3}},"required":["query"]}}},
...
async def execute_tool(name: str, args: dict, db: Session) -> str:
    if name == "search_articles": return _search_articles(db, args.get("query",""), args.get("limit",3))
...
def _search_articles(db, query, limit=3):
    ...
    arts = db.query(KnowledgeArticle).filter(KnowledgeArticle.status==1,
        or_(KnowledgeArticle.title.like(kw),KnowledgeArticle.summary.like(kw),
            KnowledgeArticle.tags.like(kw),KnowledgeArticle.content.like(kw))
...
def _export_file(args: dict) -> str:
    """Export content as PDF/DOCX/TXT"""
    from .export_service import export_pdf, export_docx, export_txt
    title = args.get("title", "Untitled")
    content = args.get("content", "")
